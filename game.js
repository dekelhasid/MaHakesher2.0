...
function showCompletedResult(result) { state.selected.clear(); state.solved = state.puzzle.groups.map((_, index) => index); state.revealAnswers = false; state.mistakes = Number(result.mistakes || 0); state.finished = true; state.saved = true; render(); $('#result-panel').hidden = false; $('#result-title').textContent = 'כל הכבוד!'; $('#result-copy').textContent = `כבר פתרתם את החידה עם ${state.mistakes} טעויות.`; setStatus('החידה כבר נפתרה על ידכם.'); }
...
function submit() {
  ...
  state.mistakes += 1; setStatus(largestMatch === 3 ? 'כמעט, שלוש מארבע.' : 'לא נכון.', true); document.querySelectorAll('.tile[aria-pressed="true"]').forEach(shake); state.selected.clear(); render(); if (state.mistakes >= MAX_MISTAKES) openDialog('#retry-dialog');
}
...
function resetGame(message = 'בחרו ארבע מילים שיש ביניהן קשר.') { state.words = scramble(state.puzzle.groups.flatMap((group, groupIndex) => group.words.map((word, wordIndex) => ({ word, groupIndex, id: `${groupIndex}-${wordIndex}` })))); state.selected.clear(); state.solved = []; state.revealAnswers = false; state.mistakes = 0; state.attempts.clear(); state.finished = false; state.saved = false; $('#result-panel').hidden = true; setStatus(message); render(); }
async function restartAfterFailure() { state.finished = true; render(); await recordAttempt(false); resetGame('ניסיון חדש לאותה חידה. בהצלחה!'); }
function chooseRetry(event) { event.preventDefault(); const choice = event.submitter?.value; $('#retry-dialog').close(); if (choice === 'solution') { void finish(false); return; } void restartAfterFailure(); }
