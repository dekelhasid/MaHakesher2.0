...
const state = { puzzle: null, words: [], selected: new Set(), solved: [], revealAnswers: false, mistakes: 0, attempts: new Set(), finished: false, saved: false, failureRecorded: false, feedbackSubmitted: false, player: getPlayer() };
...
async function loadPuzzle(requestedId = null) {
  ...
  state.selected.clear(); state.solved = []; state.revealAnswers = false; state.mistakes = 0; state.attempts.clear(); state.finished = false; state.saved = false; state.failureRecorded = false; state.feedbackSubmitted = false; $('#result-panel').hidden = true; $('#feedback-panel').hidden = true;
...
function renderPlayerButton() { const button = $('#player-button'); const name = state.player.kind === 'named' ? state.player.name.trim() : ''; button.textContent = name || 'שחקן'; button.setAttribute('aria-label', name ? `ניהול שחקן: ${name}` : 'ניהול שחקן'); }
function completionKey() { const identity = state.player.kind === 'named' ? encodeURIComponent(state.player.name.trim().toLocaleLowerCase('he')) : 'guest'; return `mahakesher.completed.${identity}.${state.puzzle.id}`; }
function saveCompletedResult(result) { if (result.solved) localStorage.setItem(completionKey(), JSON.stringify(result)); }
function showCompletedResult(result) { state.selected.clear(); state.solved = state.puzzle.groups.map((_, index) => index); state.revealAnswers = false; state.mistakes = Number(result.mistakes || 0); state.finished = true; state.saved = true; render(); $('#result-panel').hidden = false; $('#result-title').textContent = 'כל הכבוד!'; $('#result-copy').textContent = `כבר פתרתם את החידה עם ${state.mistakes} טעויות.`; setStatus('החידה כבר נפתרה על ידכם.'); showFeedbackPanel(); }
async function restoreCompletedResult() { let result = null; try { const local = JSON.parse(localStorage.getItem(completionKey()) || 'null'); if (local?.finished && local.solved) result = local; } catch { /* Ignore malformed local data. */ } if (state.player.kind === 'named') { try { const player = await findPlayer(state.player.name); const remote = player?.games?.[state.puzzle.id]; if (remote?.finished && remote.solved) result = remote; } catch (error) { console.warn('Could not restore completed game.', error); } } if (result) showCompletedResult(result); }
...
async function finish(solved) {
  ...
  $('#result-panel').hidden = false; $('#result-title').textContent = solved ? 'כל הכבוד!' : 'לא הפעם'; $('#result-copy').textContent = solved ? `פתרתם את החידה עם ${state.mistakes} טעויות.` : 'לא הצלחתם הפעם. הפתרון המלא מוצג למעלה.'; setStatus(solved ? 'החידה נפתרה.' : 'החידה הסתיימה והפתרון מוצג.', !solved); showFeedbackPanel();
}
function resultId() { return `${state.puzzle.id}-${crypto.randomUUID?.() || `${Date.now()}-${Math.random().toString(36).slice(2)}`}`; }
function resetGame(message = 'בחרו ארבע מילים שיש ביניהן קשר.') { state.words = scramble(state.puzzle.groups.flatMap((group, groupIndex) => group.words.map((word, wordIndex) => ({ word, groupIndex, id: `${groupIndex}-${wordIndex}` })))); state.selected.clear(); state.solved = []; state.revealAnswers = false; state.mistakes = 0; state.attempts.clear(); state.finished = false; state.saved = false; state.failureRecorded = false; state.feedbackSubmitted = false; $('#result-panel').hidden = true; $('#feedback-panel').hidden = true; setStatus(message); render(); }
...
function revealFailureSolution() { $('#retry-dialog').close(); state.revealAnswers = true; state.finished = true; render(); $('#result-panel').hidden = false; $('#result-title').textContent = 'לא הפעם'; $('#result-copy').textContent = 'לא הצלחתם הפעם. הפתרון המלא מוצג למעלה.'; setStatus('החידה הסתיימה והפתרון מוצג.', true); showFeedbackPanel(); }
function showFeedbackPanel() { const panel = $('#feedback-panel'); panel.hidden = false; if (!state.feedbackSubmitted) { $('#feedback-message').disabled = false; $('#feedback-submit').disabled = false; $('#feedback-status').textContent = ''; } }
async function saveFeedback() { const message = $('#feedback-message').value.trim(); const feedbackStatus = $('#feedback-status'); if (!message) { feedbackStatus.textContent = 'כתבו משוב לפני השליחה.'; return; } const button = $('#feedback-submit'); button.disabled = true; feedbackStatus.textContent = 'שולחים…'; try { await submitFeedback({ message, playerName: state.player.kind === 'named' ? state.player.name.trim() : null, playerKind: state.player.kind, puzzleId: state.puzzle.id, puzzleNumber: state.puzzle.number ?? null, puzzleTitle: state.puzzle.title, solved: state.solved.length === state.puzzle.groups.length, mistakes: state.mistakes, submittedAt: new Date().toISOString(), read: false }); state.feedbackSubmitted = true; $('#feedback-message').disabled = true; feedbackStatus.textContent = 'תודה, המשוב נשלח.'; } catch (error) { console.warn('Could not submit feedback.', error); button.disabled = false; feedbackStatus.textContent = 'לא הצלחנו לשמור את המשוב. נסו שוב.'; } }
...
async function savePlayerFromDialog(event) { event.preventDefault(); const action = event.submitter?.value; if (action === 'guest') { await continueAsGuest(); return; }
  const name = $('#player-name').value.trim(); if (!name) { showMessage('חסר שם', 'אפשר לכתוב שם או לבחור בהמשך כאורח/ת.'); return; } state.player = setPlayer({ kind: 'named', name }); renderPlayerButton(); $('#player-dialog').close(); setStatus(`שלום ${name}, בהצלחה!`); await restoreCompletedResult();
}
let nameCheck;
