...
function continueAsGuest() { state.player = setPlayer({ kind: 'guest', name: '' }); renderPlayerButton(); $('#player-dialog').close(); setStatus('ממשיכים כאורח/ת.'); }
async function savePlayerFromDialog(event) { event.preventDefault(); const action = event.submitter?.value; if (action === 'guest') { continueAsGuest(); return; }
  const name = $('#player-name').value.trim(); if (!name) { showMessage('חסר שם', 'אפשר לכתוב שם או לבחור בהמשך כאורח/ת.'); return; } state.player = setPlayer({ kind: 'named', name }); renderPlayerButton(); $('#player-dialog').close(); setStatus(`שלום ${name}, בהצלחה!`);
}
let nameCheck;
